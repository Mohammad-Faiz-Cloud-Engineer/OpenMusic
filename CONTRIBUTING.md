...
src/
  ...
  store/             Zustand stores — player, likes, recent plays, playlists
...
The player is the most complex part. `playerStore.ts` owns all playback logic — queue management, stream URL caching, shuffle/repeat, and the expo-av sound instance. If you're touching anything audio-related, read through that file first.
