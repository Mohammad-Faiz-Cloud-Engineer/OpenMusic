...
Instead, reach out to the maintainer directly:
...
**Local storage only.** Liked songs, recent plays, user-created playlists, and settings are stored on-device using AsyncStorage. Nothing is synced to a server. A vulnerability that allows one app on a device to read another app's AsyncStorage is an OS-level issue, not something this app can fix.
...
**Stream URLs are signed and short-lived.** The app fetches signed CDN URLs from the backend API. These URLs expire (the expiry is tracked in the stream cache). They're not persisted to disk because `sanitizeTrackForStorage` strips `stream_url` before anything is written to AsyncStorage.
